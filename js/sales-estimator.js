// ============================================================
// SALES ESTIMATOR — Fence Purchase flow
...
var SalesEstimator = {
  ...
  bindEvents: function () {
    ...
    tbody.addEventListener("input", function () {
      self.recalculate();
      CallFlow.onStepChange("fence-purchase", "products");
    });
    zipInput.addEventListener("input", function () {
      self.recalculate();
      if (this.value.trim().length === 5) {
        CallFlow.onStepChange("fence-purchase", "closing");
      }
    });
    ...
    document.querySelectorAll('input[name="sales-needs-delivery"]').forEach(function (r) {
      r.addEventListener("change", function () {
        ...
        self.recalculate();
        CallFlow.onStepChange("fence-purchase", "delivery");
      });
    });
  },
  ...
  recalculate: function () {
    ...
    var zip = document.getElementById("sales-zip").value.trim();
