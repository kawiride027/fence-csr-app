// ============================================================
// BARRICADE ESTIMATOR — Barricade Rental (monthly pricing)
...
var BarricadeEstimator = {
  ...
  bindEvents: function () {
    var self = this;
    document.getElementById("barricade-linear-feet").addEventListener("input", function () {
      self.recalculate();
      CallFlow.onStepChange("barricade-rental", "coverage");
    });
    document.querySelectorAll('input[name="barricade-duration"]').forEach(function (r) {
      r.addEventListener("change", function () {
        self.recalculate();
        CallFlow.onStepChange("barricade-rental", "duration");
      });
    });
    document.getElementById("barricade-zip").addEventListener("input", function () {
      self.recalculate();
      if (this.value.trim().length === 5) {
        CallFlow.onStepChange("barricade-rental", "closing");
      } else {
        CallFlow.onStepChange("barricade-rental", "delivery");
      }
    });
  },
  ...
  recalculate: function () {
    ...
    this.renderSummary({
      linearFeet: linearFeet, barricadeCount: barricadeCount, monthlyCost: monthlyCost,
      months: months, durationCost: durationCost, delivery: delivery, trueTotal: trueTotal,
      installRemovalFee: installRemovalFee, grandTotal: grandTotal
    });
  },
